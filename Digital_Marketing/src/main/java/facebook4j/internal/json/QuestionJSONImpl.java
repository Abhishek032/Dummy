...
/*package*/ final class QuestionJSONImpl extends FacebookResponseImpl implements Question, java.io.Serializable {
    ...
    private PagableList<Option> options;
    ...
    private void init(JSONObject json) throws FacebookException {
        try {
            ...
            if (!json.isNull("options")) {
                ...
                options = new PagableListImpl<Option>(0);
            }
    ...
    public PagableList<Option> getOptions() {
        return options;
    }
    ...
    /*package*/ static class OptionJSONImpl implements Option, java.io.Serializable {
    ...
    /*package*/
    static PagableList<Option> createOptionList(JSONObject json)
    throws FacebookException {
        try {
            JSONArray list = json.getJSONArray("data");
            final int size = list.length();
            PagableList<Option> options = new PagableListImpl<Option>(size, json);
            for (int i = 0; i < size; i++) {
                Option option = new OptionJSONImpl(list.getJSONObject(i));
                options.add(option);
            }
            return options;
        } catch (JSONException jsone) {
    ...
    /*package*/
    static ResponseList<Option> createOptionList(HttpResponse res, Configuration conf)
    throws FacebookException {
        try {
            if (conf.isJSONStoreEnabled()) {
                DataObjectFactoryUtil.clearThreadLocalMap();
            ...
            JSONArray list = json.getJSONArray("data");
            final int size = list.length();
            ResponseList<Option> options = new ResponseListImpl<Option>(size, json);
            for (int i = 0; i < size; i++) {
                Option option = new OptionJSONImpl(list.getJSONObject(i));
                options.add(option);
            }
