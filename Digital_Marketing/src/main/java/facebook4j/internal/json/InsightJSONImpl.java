...
/*package*/ final class InsightJSONImpl extends FacebookResponseImpl implements Insight, java.io.Serializable {
    ...
    private final class ValueJSONImpl implements Value, java.io.Serializable {
        ...
        private final class ValueEntryJSONImpl implements Entry, java.io.Serializable {
    ...
    private List<Value> createValueList(JSONObject json) throws FacebookException {
        try {
            JSONArray list = json.getJSONArray("values");
            final int size = list.length();
            List<Value> values = new ArrayList<Value>(size);
            for (int i = 0; i < size; i++) {
                Value value = new ValueJSONImpl(list.getJSONObject(i));
                values.add(value);
            }
            return values;
        } catch (JSONException jsone) {
